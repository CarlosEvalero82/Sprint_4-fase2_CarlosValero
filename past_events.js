...
async function fetchEventData() {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Set the cutoff date for past events
    const cutoffDate = new Date('2022-12-31');
    
    // Filter events that occurred before the cutoff date
    displayEvents(eventData.events.filter(event => new Date(event.date) < cutoffDate));
  ...
  function applyFilters() {
    ...
    const filteredEvents = eventData.events.filter(event => {
      ...
      const isBeforeCutoffDate = eventDate < new Date('2022-12-31');
      ...
      return isBeforeCutoffDate && matchesCategory && matchesSearch;
    });
